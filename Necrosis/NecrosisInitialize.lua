...
function Necrosis_Initialize()
	...
	-- On initialise ! Si le joueur n'est pas DÈmoniste, on cache Necrosis (chuuuut !)
	-- On indique aussi que NÈcrosis est initialisÈ maintenant
	if UnitClass("player") ~= NECROSIS_UNIT_WARLOCK then
		...
		-- On charge (ou on crÈe) la configuration pour le joueur et on l'affiche sur la console
		...
		-----------------------------------------------------------
		-- ExÈcution des fonctions de dÈmarrage
		-----------------------------------------------------------

		-- Affichage d'un message sur la console
		Necrosis_Msg(NECROSIS_MESSAGE.Interface.Welcome, "USER");
		-- CrÈation de la liste des sorts disponibles
		Necrosis_SpellSetup();
		-- CrÈation de la liste des emplacements des fragments
		Necrosis_SoulshardSetup();
		-- Inventaire des pierres et des fragments possÈdÈs par le DÈmoniste
		Necrosis_BagExplore();
		-- CrÈation des menus de buff et d'invocation
		Necrosis_CreateMenu();

		-- Lecture de la configuration dans le SavedVariables.lua, Ècriture dans les variables dÈfinies
        -- added by Sylvette
		...
		NecrosisLanguage_SliderText:SetText("Francais / English / –†—É—Å—Å–∫–∏–π / Deutsch");
		NecrosisLanguage_SliderLow:SetText("");
		NecrosisLanguage_SliderHigh:SetText("");
        
        -- ParamËtres des event range  -- sylvette
		...
		-- On rËgle la taille de la pierre et des boutons suivant les rÈglages du SavedVariables
		...
		-- On dÈfinit l'affichage des Timers ‡ gauche ou ‡ droite du bouton
		...
		-- On dÈfinit Ègalement l'affichage des tooltips pour ces timers ‡ gauche ou ‡ droite du bouton
		...
		-- On vÈrifie que les fragments sont dans le sac dÈfini par le DÈmoniste
		Necrosis_SoulshardSwitch("CHECK");

		-- Le Shard est-il vÈrouillÈ sur l'interface ?
		...
		-- Les boutons sont-ils vÈrouillÈs sur le Shard ?
		Necrosis_ButtonSetup();
		
		-- Si le dÈmoniste a une arme une main d'ÈquipÈe, on lui Èquipe le premier objet liÈ main gauche
		...
		-- Initialisation des fichiers de langues -- Mise en place Èventuelle du SMS
...
function Necrosis_SlashHandler(arg1)
	-- Blah blah blah, le joueur est-il bien un DÈmoniste ? On finira par le savoir !
	if UnitClass("player") ~= NECROSIS_UNIT_WARLOCK then
		return;
	end
