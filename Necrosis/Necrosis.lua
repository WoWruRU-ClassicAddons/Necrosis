...
-- sylvette -- stores last curse and mouse click
local SelectedCurse = 22;
local SelectedCurseMouse = "LeftButton";
...
local ShadowVulnerabilityInUse = false;
...
function Necrosis_OnLoad()
    ...
    -- added to detect ShadowVulnerability affliction and fade
    NecrosisButton:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE");
    NecrosisButton:RegisterEvent("CHAT_MSG_SPELL_AURA_GONE_OTHER");
...
function Necrosis_SetEventRange()
    if (NecrosisConfig.EventRange == nil) then
        if (GetCVar("CombatLogRangeCreature") == nil) then
            NecrosisConfig.EventRange = 30;
        else
            NecrosisConfig.EventRange = tonumber(GetCVar("CombatLogRangeCreature"));
        end
    end
    SetCVar("CombatLogRangeCreature", NecrosisConfig.EventRange);
    SetCVar("CombatLogRangeParty", NecrosisConfig.EventRange);
    SetCVar("CombatLogRangePartyPet", NecrosisConfig.EventRange);
    SetCVar("CombatLogRangeFriendlyPlayers", NecrosisConfig.EventRange);
    SetCVar("CombatLogRangeFriendlyPlayersPets", NecrosisConfig.EventRange);
    SetCVar("CombatDeathLogRange", NecrosisConfig.EventRange);
    SetCVar("CombatModeMaxDistance", NecrosisConfig.EventRange*1.0);
end
...
local updateIntervalMinimum = 1.0;
...
function Necrosis_OnUpdate(elapsed)
    ...
    -- sylvette added minimum update period.
    timeSinceLastUpdate = timeSinceLastUpdate + elapsed;
    if (timeSinceLastUpdate <= updateIntervalMinimum) then
        return;
    end
	...
	local GraphicalTimer = {texte = {}, TimeMax = {}, Time = {}, titre = {}, temps = {}, Gtimer = {}, ColorVulnerable = {}};
	if SpellTimer then
...
function Necrosis_OnEvent(event)
    ...
    -- sylvette -- check for vulnerability and set the panel on or off.
    if (event == "CHAT_MSG_SPELL_AURA_GONE_OTHER" or event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE" or event == "PLAYER_TARGET_CHANGED") then
        if UnitExists("target") and UnitCanAttack("player", "target") and not UnitIsDead("target") then
            if (Necrosis_UnitHasEffect("target", "Shadow Vulnerability")) then
                if (ShadowVulnerabilityInUse == false) then
                    ShadowVulnerabilityInUse = true;
                    ShowUIPanel(NecrosisShadowVulnerabilityButton);
                    if NecrosisConfig.Sound then 
                        PlaySoundFile(NECROSIS_SOUND.ShadowVulnerability);
                    end
                end
            else
                if (ShadowVulnerabilityInUse) then
                    ShadowVulnerabilityInUse = false;
                    HideUIPanel(NecrosisShadowVulnerabilityButton);
                end
            end
        elseif (ShadowVulnerabilityInUse) then
            ShadowVulnerabilityInUse = false;
            HideUIPanel(NecrosisShadowVulnerabilityButton);
        end
    end
...
function Necrosis_RegisterManagement(RegistrationType)
	if RegistrationType == "IN" then
		...
		NecrosisButton:RegisterEvent("TRADE_SHOW");
		NecrosisButton:RegisterEvent("TRADE_CLOSED");
		NecrosisButton:RegisterEvent("CHAT_MSG_SPELL_FRIENDLYPLAYER_DAMAGE");
		NecrosisButton:RegisterEvent("CHAT_MSG_SPELL_SELF_DAMAGE");
        -- added to detect ShadowVulnerability affliction and fade
		NecrosisButton:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE");
		NecrosisButton:RegisterEvent("CHAT_MSG_SPELL_AURA_GONE_OTHER");
		...
		NecrosisButton:UnregisterEvent("CHAT_MSG_SPELL_FRIENDLYPLAYER_DAMAGE");
		NecrosisButton:UnregisterEvent("CHAT_MSG_SPELL_SELF_DAMAGE");
		NecrosisButton:UnregisterEvent("UNIT_AURA");
        -- added to detect ShadowVulnerability affliction and fade
		NecrosisButton:UnregisterEvent("CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE");
		NecrosisButton:UnregisterEvent("CHAT_MSG_SPELL_AURA_GONE_OTHER");
...
function Necrosis_ChangeDemon()
	...
	if (Necrosis_UnitHasEffect("pet", NECROSIS_SPELL_TABLE[10].Name)) then
		if (not DemonEnslaved) then
			DemonEnslaved = true;
			SpellGroup, SpellTimer, TimerTable = Necrosis_InsertTimerParTable(10, "","", SpellGroup, SpellTimer, TimerTable, ShadowVulnerabilityInUse);
		end
	else
...
function Necrosis_SelfEffect(action)
	if action == "BUFF" then
		...
		if arg1 == NECROSIS_TRANSLATION.SacrificeGain then
			SpellGroup, SpellTimer, TimerTable = Necrosis_InsertTimerParTable(17, "", "", SpellGroup, SpellTimer, TimerTable, ShadowVulnerabilityInUse);
		end
...
function Necrosis_SpellManagement()
	...
	if (SpellCastName) then
		...
		if (SpellCastName == NECROSIS_SPELL_TABLE[11].Name) then
			...
			SpellGroup, SpellTimer, TimerTable = Necrosis_InsertTimerParTable(11, SpellTargetName, "", SpellGroup, SpellTimer, TimerTable, ShadowVulnerabilityInUse);
			...
			for spell=1, table.getn(NECROSIS_SPELL_TABLE), 1 do
				if SpellCastName == NECROSIS_SPELL_TABLE[spell].Name and not (spell == 10) then
					...
					for thisspell=1, table.getn(SpellTimer), 1 do
						if SpellTimer[thisspell].Name == SpellCastName
							...
							if spell ~= 9 or (spell == 9 and not Necrosis_UnitHasEffect("target", SpellCastName)) then
                                ...
                                SpellTimer[thisspell].ColorVulnerable = ShadowVulnerabilityInUse;
						...
						if SpellTimer[thisspell].Name == SpellCastName and spell == 9
							...
							then
							SpellTimer, TimerTable = Necrosis_RetraitTimerParIndex(thisspell, SpellTimer, TimerTable, false);
							SortActif = false;
							break;
						end	
					...
					if (NECROSIS_SPELL_TABLE[spell].Type == 4) or (spell == 16) then
						for thisspell=1, table.getn(SpellTimer), 1 do
							...
							if SpellTimer[thisspell].Type == 4
								...
								and SpellTimer[thisspell].TargetLevel == SpellTargetLevel
								then
								SpellTimer, TimerTable = Necrosis_RetraitTimerParIndex(thisspell, SpellTimer, TimerTable, ShadowVulnerabilityInUse);
								break;
							end
						end
						SortActif = false;
					...
					if not SortActif
						...
						SpellGroup, SpellTimer, TimerTable = Necrosis_InsertTimerParTable(spell, SpellTargetName, SpellTargetLevel, SpellGroup, SpellTimer, TimerTable, ShadowVulnerabilityInUse);
						break;
					end
				end
			end
...
function Necrosis_Toggle(button)
    ...
    -- get the event range in case some other mod has set it.
    if (GetCVar("CombatLogRangeCreature") == nil) then
        if (NecrosisConfig.EventRange == nil) then
            NecrosisConfig.EventRange = 30;
            Necrosis_SetEventRange();
...
function Necrosis_UseItem(type,button)
	...
	elseif (type == "Healthstone") then
		...
		if HealthstoneOnHand then
			...
			if NecrosisTradeRequest then
				...
				TradingNow = 3;
				return;
			elseif (NecrosisConfig.StoneTrade or (not(NecrosisConfig.StoneTrade) and not(PlayerCombat)))
                            and (UnitExists("target") and UnitIsPlayer("target") 
                            and UnitIsFriend("player", "target") and UnitName("target") ~= UnitName("player")) then
				PickupContainerItem(HealthstoneLocation[1], HealthstoneLocation[2]);
	        		if ( CursorHasItem() ) then
	...
	elseif (type == "Mount") then
		...
		if NECROSIS_SPELL_TABLE[2].ID ~= nil then
			...
			Necrosis_OnUpdate(updateIntervalMinimum+1);
...
function Necrosis_CurseMenu(button)
	...
	if not CurseMenuShow then
		CurseShow = false;
		CurseVisible = false;
        -- sylvette - set the texture of the curse menu to the select curse.
        Necrosis_SelectCurseMenuTexture();
...
-- casts Banish
function Necrosis_BoundBanishCast()
    Necrosis_BuffCast(9);
end

-- casts Enslave
function Necrosis_BoundEnslaveCast()
    Necrosis_PetCast(35);
end

-- casts fear
function Necrosis_BoundFearCast()
    CastSpell(NECROSIS_SPELL_TABLE[13].ID, "Spell");
    ---Necrosis_CastSpellOnEnemy(13, "LeftButton");
end

-- casts HoT
function Necrosis_BoundHoTCast()
    Necrosis_CastSpellOnEnemy(29, "LeftButton");
end

-- casts Drain Soul (Rank 1)
function Necrosis_BoundDrainSoulCast()
    CastSpellByName("Drain Soul(Rank 1)");
end

-- optional ScaledLifeTap
function Necrosis_CastScaledLifeTap()
    -- optional mod named ScaledLifeTap
    if (ScaledLifetap_CastLifeTap) then
        ScaledLifetap_CastLifeTap();
    else
        Necrosis_BuffCast(41);  -- not so scaled.
    end
end

-- sylvette
-- default curse selection and setup.
-- this is called by the bind key
function Necrosis_BoundCurseCast()
    Necrosis_CastSpellOnEnemy(SelectedCurse, SelectedCurseMouse);
end

-- sylvette
-- sets the default curse and the texture of the curse menu button
-- middle button also set pet attack
-- right button sets amplify curse to auto-trigger
-- Necrosis message is printed indicating current curse and modifiers.
function Necrosis_SetDefaultCurse(type, click)
    SelectedCurse = type;
    SelectedCurseMouse = click;
    Necrosis_SelectCurseMenuTexture();
    local mess = "<lightYellow>"..NECROSIS_SPELL_TABLE[SelectedCurse].Name;
    if (NECROSIS_SPELL_TABLE[SelectedCurse].Rank) then
        mess = "<lightYellow>"..NECROSIS_SPELL_TABLE[SelectedCurse].Name.."(Rank "..NECROSIS_SPELL_TABLE[SelectedCurse].Rank..")";
    end
    Necrosis_Msg(mess..NECROSIS_MESSAGE.Interface.CurseSelection, "USER");
    if (click == nil) then
        return;
    elseif (click == "RightButton") then
        Necrosis_Msg(NECROSIS_MESSAGE.Interface.CurseAmplify, "USER");
    elseif (click == "MiddleButton") then
        Necrosis_Msg(NECROSIS_MESSAGE.Interface.CursePetAttack, "USER");
    end
end

-- sylvette added code.
-- sets the CurseMenu to show the selected (or default) curse.
function Necrosis_SelectCurseMenuTexture()
    NecrosisCurseMenuButton:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\"..NECROSIS_SPELL_TABLE[SelectedCurse].TexturePrefix.."-01");
    NecrosisCurseMenuButton:SetHighlightTexture("Interface\\AddOns\\Necrosis\\UI\\"..NECROSIS_SPELL_TABLE[SelectedCurse].TexturePrefix.."-02");
end

-- sylvette commented (by rename) old code.
-- Gestion des casts du menu des curses
-- renamed from "Necrosis_CurseCast" to "Necrosis_CastSelectedCurse" otherwise the same
function Necrosis_CastSpellOnEnemy(type, click)
    -- if the unit is MCd then allow fear to be cast otherwise assist
	if (UnitIsFriend("player","target") and UnitPlayerControlled("target") and type ~= 13) then
		AssistUnit("target");
	end
...
function NecrosisTimer(nom, duree)
	...
	SpellGroup, SpellTimer, TimerTable = NecrosisTimerX(nom, duree, truc, Cible, Niveau, SpellGroup, SpellTimer, TimerTable, ShadowVulnerabilityInUse);
end
